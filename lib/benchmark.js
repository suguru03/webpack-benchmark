...
const tasks = _.chain(fs.readdirSync(dirpath))
  ...
  .flatten()
  .filter(({ name, title }) => re.test(`${name}:${title}`))
  .value();

console.log(`${list ? "Showing" : "Executing"} tasks filtered by ${re}`);
_.forEach(tasks, ({ name, title }) => console.log(`[${name}:${title}]`));
if (list) {
  return;
}
