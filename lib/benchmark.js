...
async.series([
	async.apply(prepare, versions),
	async.apply(execute, versions)
]);
...
function execute(versions, callback) {
	...
	async.forEachSeries(tasks, ({ name, title, task }, done) => {
			...
			(list, next) => {
				...
				suite.on("complete", () => {
					...
					_.forEach(result, ({ name, mean }, index) => {
						...
						mean *= 1000;
						console.log(`[${++index}] "${name}" ${mean.toPrecision(3)}ms [${rate.toPrecision(3)}]`);
					});
					next(null, result);
				})
				.run({ async: true })
